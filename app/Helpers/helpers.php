...
if (!function_exists('find_zone')) {
    ...
    {
        // $zone = Zone::active()->whereHas('zoneBound', function ($query) use ($lat,$lng) {
        //     $query->where('north', '>=', $lat);
        //     $query->where('south', '<=', $lat);
        //     $query->where('east', '>=', $lng);
        //     $query->where('west', '<=', $lng);
        // })->first();
...
if (!function_exists('get_distance_matrix')) {
    function get_distance_matrix($pick_lat, $pick_lng, $drop_lat, $drop_lng, $traffic = fals)
    {
        ...
        $args = [
          ...
          'key' => 'AIzaSyDsgTHjo-lusijguNf8XO8aLNyYHe9mRE4'
        ];
...
if (!function_exists('php_to_node_structure')) {
    /**
     * Check the given plain value against a hash.
     *
     * @param  uuid  $id
     ...
     * @return bool
     */
    function php_to_node_structure($id, $user_type, $message, $event)
    {
        $structure = array();
        $structure['id']= $id;
