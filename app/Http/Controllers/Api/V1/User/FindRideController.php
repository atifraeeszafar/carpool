...
class FindRideController extends BaseController
{
    ...
    */
    public function findRide(FindRideRequest $request)
    {
        ...
        $haversine = "(6371 * acos(cos(radians($request->pickup_lat)) * cos(radians(pickup_lat)) * cos(radians(pickup_lng) - radians($request->pickup_lng)) + sin(radians($request->pickup_lat)) * sin(radians(pickup_lat))))";
        ...
        $rider_places = OfferedRidePlaceStop::select('offered_place_stops.*')->selectRaw("{$haversine} AS distance")
